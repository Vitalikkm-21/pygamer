...
# Load sound
flip_sound = pygame.mixer.Sound("flip_sound.mp3")  # Replace "flip_sound.mp3" with your sound file
...
def load_animation_frames(file_name_prefix, num_frames):
    ...
    for i in range(num_frames):
        if i <= 9:
            frame = pygame.image.load(file_name_prefix + "00" + str(i) + ".png")
        elif i <= 99:
            frame = pygame.image.load(file_name_prefix + "0" + str(i) + ".png")
        else:
            frame = pygame.image.load(file_name_prefix + str(i) + ".png")
        frames.append(frame)
    return frames
...
def draw_animation():
    ...
    if not animation_finished:
        if flip_result not in animation_frames:
            flip_result = 'heads'  # Change 'Heads' to 'heads'
        if flip_result == 'heads':  # Change 'Heads' to 'heads'
            frame = animation_frames['heads'][current_frame]  # Change 'Heads' to 'heads'
        else:
            frame = animation_frames['tails'][current_frame]  # Change 'Tails' to 'tails'
        ...
        if current_frame >= len(animation_frames[flip_result]):
            ...
            if flip_result == 'heads':
                ...
                # Воспроизводим второй звук, если выпала "решка"
                flip_sound_2.play()
...
def draw_buttons():
    ...
    if goal == -1:
        pygame.draw.rect(screen, BUTTON_COLOR, level_button_1)
        pygame.draw.rect(screen, BUTTON_COLOR, level_button_2)
        pygame.draw.rect(screen, BUTTON_COLOR, level_button_3)
...
background_image = pygame.image.load("Logo_astana_it_university.png")  # Replace "background_image.png" with your image file
...
while running:
    ...
    if goal != -1:
        ...
        render_text("You must to choose the level!!!!", fONT, screen, (325, 210), (0, 0, 0))
    render_text(ball, fONT, screen, (170,150), (0, 0, 0))
    if ball == 10:
        render_text(ball_3, fONT, screen, (500, 100), (0, 0, 0))
    if goal == mistake:
        render_text(ball_4, fONT, screen, (500, 100), (0, 0, 0))
    handle_events()
    if flip_result:
        draw_animation()
